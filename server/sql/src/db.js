...
export class Database {
    ...
    async insert(pokemon){
        ...
        const query = await this.pool.query(
            `INSERT INTO generated_pokemon (name, type, powers, characteristics, image_url, doodle_source) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [name, type, powers, characteristics, image_url, doodle_source]
        );
        return query.rows[0];
    }
    ...
    return result.rows;
     ...
     async like(id) {
    const result = await this.pool.query(
      `UPDATE generated_pokemon SET like_count = like_count + 1 WHERE id = $1
    ...
    const result = await this.pool.query(
      `SELECT id, name, type, powers, characteristics, image_url, doodle_source, like_count, action_images
       FROM generated_pokemon WHERE id = $1`,
      [id]
    )
    return result.rows[0] || null
  }
